Swiftbat (`swiftbat_py` on pypi) is a set of Python library routines and command-line utilities
that have been developed for the purpose of retrieving, analyzing, and displaying data from
NASA's Swift spacecraft, especially the data from the Swift Burst Alert Telescope (BAT).

Development started before the launch of Swift for private use by the author and was how
he learned Python. As a result, it is not a well-packaged, well-written, coherent library.

All of this data is available from the Swift data archive, but some routines in this library
use other access methods that are not available to the general public. These routines will
not be useful except to Swift team members.
...
If your man-in-the-middle proxy/VPN is giving you certificate errors
when you try to get pointing information or other data services, use the
`swutil.trust_proxy()` function.